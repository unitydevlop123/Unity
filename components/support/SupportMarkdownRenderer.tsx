...
import SupportProfessionalTable from './SupportProfessionalTable';
import { formatUnityDevText } from '../../utils/textSanitizer';
import './SupportMarkdownRenderer.css';
...
const getStyleForTable = (node: any): 'minimal' | 'tech' | 'grid' | 'comparison' | 'corporate' | 'modern' | 'colorful' | 'dark' | 'gradient' | 'striped' | 'glass' | 'bento' => {
  try {
...
export const markdownComponents = {
  ...
  // Headers
  h1: ({node, ...props}: any) => <h1 className="text-3xl font-bold mt-8 mb-4 tracking-tight" {...props} />,
  h2: ({node, ...props}: any) => <h2 className="text-2xl font-bold mt-6 mb-3 tracking-tight" {...props} />,
  h3: ({node, ...props}: any) => <h3 className="text-xl font-bold mt-5 mb-2 tracking-tight" {...props} />,
  h4: ({node, ...props}: any) => <h4 className="text-lg font-bold mt-4 mb-2 tracking-tight" {...props} />,
  h5: ({node, ...props}: any) => <h5 className="text-base font-bold mt-3 mb-1 tracking-tight" {...props} />,
  h6: ({node, ...props}: any) => <h6 className="text-sm font-bold mt-2 mb-1 tracking-tight uppercase" {...props} />,
  ...
  code: ({node, inline, className, children, ...props}: any) => {
    ...
    return !inline ? (
      <div className="my-6 rounded-xl overflow-hidden border border-white/10 shadow-2xl bg-zinc-700/90 backdrop-blur-sm">
        <div className="flex items-center justify-between px-4 py-1.5 bg-zinc-800/80 text-zinc-300 text-[10px] font-sans border-b border-white/5 uppercase tracking-wider">
          ...
          <button 
            ...
            title="Copy"
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2 2v1"></path></svg>
            <span className="font-medium">Copy</span>
          </button>
        </div>
        <pre className="p-4 overflow-x-auto font-mono text-sm leading-relaxed text-zinc-100 !m-0 !bg-transparent !border-none !shadow-none !rounded-none">
  ...
  table: ({ node, ...props }: any) => {
    ...
    return (
      <div className="table-scroll-wrapper my-8 overflow-x-auto rounded-xl border border-white/10 shadow-xl">
        <SupportProfessionalTable style={tableStyle} {...props} />
      </div>
    );
  },
...
const SupportMarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  
  // Helper to process content and replace custom image tags
  const processContent = (text: string) => {
    // 0. Apply UnityDev Ultra-Cleaner (Sanitizer Script)
    let processed = formatUnityDevText(text);

    // Protect math blocks from being mangled by regexes
...
export default SupportMarkdownRenderer;
