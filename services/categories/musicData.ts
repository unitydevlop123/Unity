...
export const getMusicVideos = async (page: number = 1): Promise<Video[]> => {
  try {
    let apiVideos: Video[] = [];
    let aiVideos: Video[] = [];
    ...
    try {
      const [apiRes, aiRes] = await Promise.all([
        fetchAndFilterVideos(
          'Music video official 4k pop rap r&b',
          page,
          180,
          categoryFilters.music
        ).catch(() => []),
        firebaseRest.getGlobalCategoryVideos('music').catch(() => [])
      ]);
