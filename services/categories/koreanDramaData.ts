...
export const getKoreanDramaVideos = async (page: number = 1): Promise<Video[]> => {
  try {
    let apiVideos: Video[] = [];
    let aiVideos: Video[] = [];
    ...
    try {
      const [apiRes, aiRes] = await Promise.all([
        fetchAndFilterVideos(
          'Korean drama kdrama full episode',
          page,
          300,
          categoryFilters.korean_drama
        ).catch(() => []),
        firebaseRest.getGlobalCategoryVideos('korean_drama').catch(() => [])
      ]);
    ...
    console.error("Error fetching Korean Drama videos:", error);
    return [];
  }
};

export const koreanDramaVideos: Video[] = [];
